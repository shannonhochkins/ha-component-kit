...
import callApiExample from './examples/callApi.code.tsx?raw';
import callApiContext from './examples/callApi.context.code.ts?raw';
import callApiSnapshot from './examples/callApi.snapshot.code.ts?raw';
...
### Example Usage
Use the context helper (stable) or snapshot from the store for non-render code.
...
<Source dark language="ts" code={callApiContext} />
<Source dark language="ts" code={callApiSnapshot} />
