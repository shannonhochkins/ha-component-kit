...
import { DummyComponentReturn, DummyComponentReturnFloors, DummyComponentAreas, DummyComponentDevices, DummyComponentServices, DummyComponentEntitiesRegistryDisplay } from './examples/DummyComponent';
...
2. For programmatic (non-hook) access you can still do: `const { entitiesRegistryDisplay } = useHass.getState();` but the hook gives you reactivity

### When to Prefer Direct Store Access

Use the hook for component-level reactive subscriptions. Use `useHass.getState()` when:
...
Type narrowing occurs automatically. Hover over `entitiesRegistryDisplay` in your editor to see full `EntityRegistryDisplayEntry` structure without casts.
