...
This hook is designed to retrieve an entity and return all it's information including history (if not disabled) and the actions for the entity.
...
### Using actions from entity
The `useEntity()` hook is integrated with the `useAction` hook so you can use it directly from the entity object.
...
Some actions return a response, like the calendar `getEvents` service, you can access this response value by adding `returnResponse` to the service call.
