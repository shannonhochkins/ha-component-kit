...
export default function ClimateState({
  entity,
  className,
  ...rest
}: StateProps) {
  ...
  const computeAttribute = useCallback((attribute: string) => computeAttributeValueDisplay(entity, config as HassConfig, entities, attribute), [entity, config, entities]);
  ...
  const _computeCurrentStatus = (): string | undefined  => {
    if (
      ...
      entity.attributes.current_humidity != null
    ) {
  ...
  const _computeTarget = (): string => {
    ...
    if (
      entity.attributes.target_humidity_low != null &&
      entity.attributes.target_humidity_high != null
    ) {
  ...
  return (<Wrapper className={`action-type-climate ${className}`} {...rest}>
    ...
    {currentStatus && !isUnavailable
      ? <div className="current">
        {localize("currently")}:
        <div className="unit">{currentStatus}</div>
      </div>
    : null}
  </Wrapper>);
}