...
const Description = styled.div`
  ...
  gap: 0.5rem;
`;
type Extendable = Omit<ClimateControlsProps, 'onClick'> & MotionProps & Omit<React.ComponentPropsWithoutRef<'button'>, 'onClick'>;
...
/** The ClimateCard is a card to easily interact with climate entities */
export function ClimateCard({
  ...
  return (<>
    <Ripples borderRadius="1rem" whileTap={{ scale: 0.9 }}>
      <StyledClimateCard {...longPressEvent} layoutId={`${_entity}-climate-card`} {...rest} onClick={() => {
        ...
        <LayoutBetween>
          <Description>
            <Icon>{entityIcon || domainIcon}</Icon> {title} - {titleValue}
          </Description>
        ...
        <Row fullWidth gap="0.5rem" wrap="nowrap">
          {(hvacModes || hvac_modes || [])
            .concat()
            .map((mode) => <FabCard size={35} iconColor={currentMode === mode ? activeColors[mode] : undefined} key={mode} title={mode} active={currentMode === mode} icon={icons[mode]} onClick={() => {
