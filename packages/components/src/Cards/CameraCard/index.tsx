...
import { PreloadImage, fallback, Row, mq, FabCard, ButtonGroup, ButtonGroupProps } from "@components";
import { ErrorBoundary } from "react-error-boundary";
import { CameraStream } from './stream';
import { type VideoState } from './players';
import { Icon } from '@iconify/react';

type Extendable = Omit<React.ComponentProps<"div">, "onClick" | "ref"> & MotionProps;
...
function _CameraCard({
  ...
  const _headerSensors = useMemo(() => headerSensors?.map(sensor => ({
  ...
  >
    <Header justifyContent="space-between" gap="0.5rem">
      <Row justifyContent="flex-start" gap="0.5rem">
        <StateFabCard active borderRadius={10} disableScaleEffect size={30} noIcon>
      ...
      {!hideName && <CameraName>{name ?? camera.attributes.friendly_name ?? camera.entity_id}</CameraName>}
    </Header>
    {!hideFooter && <Footer justifyContent="space-between" gap="0.5rem" wrap="nowrap">
      {_headerSensors && <Row justifyContent="flex-start" gap="0.5rem">
        <ButtonGroup
          cssStyles={`
          button {
      ...
      {!hideViewControls && <Row justifyContent="flex-start" gap="0.5rem">
        <ButtonGroup
        cssStyles={`
          button {
            height: ${DEFAULT_ICON_BUTTON_SIZE}px !important;
    ...
    {_view !== 'live' && (poster.url || mjpeg.url) && <PreloadImage
      onLoad={onImageLoad}
