...
export function ThemeControls({
  ...
  return (
    <Column fullWidth gap="1rem" wrap="nowrap" style={{
      padding: '1rem 0',
    }}>
      <Row fullWidth gap="1rem" wrap="nowrap">
        <RangeSliderBox>
          <RangeSlider label="Shade Tint" description="Will change the hue tint for the shade colors" min={0} max={1} step={0.05} value={t} onChange={(value) => {
            setTint(value);
          }} />
        </RangeSliderBox>
        <RangeSliderBox>
          <RangeSlider label="Contrast Threshold" description="Changes output calculation for text color" min={0} max={100} step={1} value={c} onChange={(value) => {
      ...
      <Row fullWidth gap="1rem" wrap="nowrap">
        <RangeSliderBox>
          <RangeSlider label="Saturation" description="Control the saturation of the color" min={0} max={100} value={s} onChange={(value) => {
            setSat(value);
          }} />
        </RangeSliderBox>
        <RangeSliderBox>
          <RangeSlider label="Lightness" description="Control how bright the primary colors are" min={0} max={100} value={l} onChange={(value) => {
            setLight(value);
          }} />
        </RangeSliderBox>
      </Row>
      <RangeSliderBox>
        <RangeSlider label="Hue" description="Change the overall hue of the colors" min={0} max={360} value={h} onChange={(value) => {
          setHue(value);
        }} />
      </RangeSliderBox>
      ...
      <Row justifyContent="flex-start" fullWidth gap="1rem">
        ...
        <FabCard tooltipPlacement="right" title={dark ? 'Change to light mode' : 'Change to dark mode'} icon={!dark ? 'ph:sun' : 'ph:moon'} onClick={() => {
          setDark(!dark);
        }} />
      </Row>
      <Row justifyContent="flex-start" fullWidth><Alert>Hover over the pallette's below to see the respective css variables.</Alert></Row>
      <Row wrap="nowrap" fullWidth justifyContent="flex-start">
        {LIGHT.map((color, index) => {
          return <Column gap="1rem" key={color}>
            <Title>{index === 0 ? 'Primary' : <>&nbsp;</>}</Title>
            <Tooltip placement="top" title={<Column alignItems="flex-start">
        ...
        {DARK.map((color) => {
          return <Column key={color}  gap="1rem">
            <Title>{<>&nbsp;</>}</Title>
            <Tooltip placement="top" title={<Column alignItems="flex-start">
              <span>background-color: var(--ha-{color});</span>
              <span>color: var(--ha-{color}-contrast);</span>
